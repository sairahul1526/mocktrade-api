...
// AmountGet .
func AmountGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + amountTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + amountTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
func AmountAdd(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := requiredFiledsCheck(body, amountRequiredFields)
	if len(fieldCheck) > 0 {
	...
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Amount added", "")
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
// AmountUpdate .
func AmountUpdate(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	for _, field := range amountRequiredFields {
	...
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Amount updated", "")
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
