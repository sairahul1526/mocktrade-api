...
func PositionGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + positionTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + positionTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
func PositionAdd(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := requiredFiledsCheck(body, positionRequiredFields)
	if len(fieldCheck) > 0 {
	...
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Position added", "")
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
func PositionUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, field := range positionRequiredFields {
	...
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Position updated", "")
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
